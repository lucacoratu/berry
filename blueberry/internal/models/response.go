...
type CranberryAPIError struct {
	Detail string `json:"detail"` //The message of the error
}

// Convert from json into the APIError structure
func (err *CranberryAPIError) FromJSON(r io.Reader) error {
	d := json.NewDecoder(r)
	return d.Decode(err)
}

// Convert to json the APIError structure
func (err *CranberryAPIError) ToJSON(w io.Writer) error {
	e := json.NewEncoder(w)
	return e.Encode(err)
}
