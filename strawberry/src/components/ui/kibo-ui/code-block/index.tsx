...
const markerRegex = /(?:#|\/\/)\s*\[!code\s+highlight(?::(\d+))?\]/;

function getTextFromHast(node: any): string {
  if (!node) return "";
...
function replaceInHast(node: any, re: RegExp, replacement = "") {
  if (!node) return;
  if (node.type === "text") {
    node.value = String(node.value ?? "").replace(re, replacement);
    return;
  }
  if (Array.isArray(node.children)) {
    node.children.forEach((c: any) => replaceInHast(c, re, replacement));
  }
}

const transformerHttpHighlight: ShikiTransformer = {
  name: "http-line-highlight",
  enforce: "post", // run after built-in transformers
  // per-code-block state:
  pre() {
    // reset state for each code block
    (this as any)._pendingHighlightLines = 0;
    (this as any)._foundAnyHighlight = false;
  },
  line(node: any, lineNumber: number) {
    const text = getTextFromHast(node).trim();

    // 1) full-line marker like: "// [!code highlight:3]" or "# [!code highlight:2]"
    const fullLineMatch = text.match(/^(?:#|\/\/)\s*\[!code\s+highlight(?::(\d+))?\]$/);
    if (fullLineMatch) {
      // remove marker content from this line (so it won't render)
      replaceInHast(node, markerRegex, "");
      const count = Number(fullLineMatch[1] ?? 1);
      (this as any)._pendingHighlightLines = count;
      (this as any)._foundAnyHighlight = true;
      return; // don't mark the comment line itself highlighted
    }

    // If we have pending "next N lines" to highlight, apply it
    if ((this as any)._pendingHighlightLines > 0) {
      this.addClassToHast(node, "highlighted");
      (this as any)._pendingHighlightLines--;
      (this as any)._foundAnyHighlight = true;
      // also strip any trailing inline markers just in case
      replaceInHast(node, markerRegex, "");
      return;
    }

    // 2) trailing marker in same line, e.g. Authorization: Bearer token123 # [!code highlight]
    if (markerRegex.test(text)) {
      // mark the line
      this.addClassToHast(node, "highlighted");
      (this as any)._foundAnyHighlight = true;

      // remove marker text from the HAST children so it doesn't print
      replaceInHast(node, markerRegex, "");
      return;
    }

    // otherwise nothing to do for this line
  },
  // pre(node: any) {
  //   // after processing lines, add a flag to the pre if any highlighted lines existed
  //   if ((this as any)._foundAnyHighlight) {
